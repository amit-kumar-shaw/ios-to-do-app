...
//        }.store(in: &cancelables)
    ...
    func loadProject(){
        projectSubscription = Firestore.firestore().document("projects/\(projectId)").addSnapshotListener({ docSnapshot, error in
            if error != nil {
                self.showAlert = true
                self.error = error
    ...
    func loadList(filter: FilterType){
        ...
        let collectionRef = Firestore.firestore().collection("todos").whereField("projectId", isEqualTo: projectId)
